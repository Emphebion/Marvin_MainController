...
**Depends on:** [Phase 2](260411_phase2_architecture.md) — requires `BaseGame` interface and `_LineGame.py` extraction
**Followed by:** Phase 3b (below), then [Phase 4](260411_phase4_mqtt.md)
...
- **Colour system:** Rune colours are loaded from `marvinconfig.txt [RuneGame] runeColorL1/L2/L3` (colour names), resolved to RGB via `colorsLED` at game start. See [overview — Colour System](260407_overhaul_plan.md#colour-system). These colours are also settable via MQTT in Phase 4 — see [phase4_mqtt.md — Config Field Mapping](260411_phase4_mqtt.md#config-field-mapping-mqtt--internal).
- **BFS algorithm:** the segment-directed BFS that drives the reveal is fully specified in [docs/rune_bfs_fix.md](260406_rune_bfs_fix.md). Read that document before touching `_RuneGame._bfs_order()`.
...
- `_bfs_order(rune)` — segment-directed BFS returning `list[list[tuple]]` (layers). Full algorithm: [docs/rune_bfs_fix.md](260406_rune_bfs_fix.md).
...
- **Well mechanic:** see [overview — Well Mechanic](260407_overhaul_plan.md#well-mechanic) — `[State6] source` in `marvinconfig.txt` is confirmed dead config.
